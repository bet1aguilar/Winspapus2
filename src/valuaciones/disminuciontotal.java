 ...
 * @author Betmart
 */
public class disminuciontotal extends javax.swing.JDialog {
    ...
    public final void cargapartidas(){
       ...
       public Class getColumnClass(int columna)
        ...
        try {
            ...
            Logger.getLogger(disminuciontotal.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    private void initComponents() {
        ...
        jLabel1.setText("Agregar Partidas a Dismininución total");
        ...
        try {
            ...
            for(int i=0; i<contsel;i++){
                ...
                String datospartida = "SELECT id, IF(precunit=0, precasu, precunit) AS precio "
                        + ", cantidad FROM mppres "
                        + "WHERE numero ="+partidas[i]+" AND (mpre_id='"+pres+"' "
...
public void busca(){
       ...
       public Class getColumnClass(int columna)
        ...
        try {
            Statement s = (Statement) conex.createStatement();
            String sql = "SELECT id, descri, numero, numegrup, precunit, precasu FROM mppres WHERE numero NOT "
                    + "IN(SELECT numepart FROM dvalus WHERE (mpre_id='"+pres+"' OR mpre_id "
                    + "IN (SELECT id FROM mpres WHERE mpres_id='"+pres+"'))) AND (mpre_id='"+pres+"' OR mpre_id IN "
